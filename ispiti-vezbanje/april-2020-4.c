//napisati c kod za funkciju za kvadriranje vrendosti svvih elemenata stabla.
//napsiati odgovarajući pseudokod za dodavanje i brisanje elemenata iz min hipa
...
void kvadriranje(struct node * root){
    if(root){
        root -> key =  pow(root-> key, 2); 
        kvadriranje(root-> left);
        kvadriranje(root-> right);
    }
}
